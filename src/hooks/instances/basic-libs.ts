...
import { DefaultResourceLoader, Resource } from '../../resource-loader';
...
function getBasicLibsConfig(scope: HookScope<HookBasicLibsOpts>) {
  ...
  const resourceLoader = base.getConfig('resourceLoader') || DefaultResourceLoader;
  const basicLibs = base.getConfig('basicLibs') as Resource[];
  const useSystem = base.getConfig('useSystem') as string[];

  const basicLibUseSystem = useSystem.findIndex((n) => n === 'basicLibs') > -1;
  ...
  return {
    async beforeRouting(scope: HookScope<HookBasicLibsOpts>) {
      ...
      if (!basicLibsConfig.base.getData('basicLibs')) {
        basicLibsConfig.basicLibs.forEach((r) => {
          basicLibsConfig.resourceLoader.mount(r, scope, { useSystem: basicLibsConfig.useSystem });
        });
